...
{                     
  ...
  float Normi(), Normr(), *in_vector, *in_test_vector, *vector(), *labs, datamin, datamax, *test_labs;
  float *orig_vector, p, pmax, ival, mlikedenom, in_value, *test_vector;
  double mu[MAX_SEGMENTS], sd[MAX_SEGMENTS],test_mu[MAX_SEGMENTS], test_sd[MAX_SEGMENTS];
  double loglik_old, loglik_new, conv_crit, segment_probs[MAX_SEGMENTS], test_segment_probs[MAX_SEGMENTS];
  ...
  int i, j, k, iseg, segment_number, n,m;
  ...
  int cardinality[MAX_SEGMENTS],test_cardinality[MAX_SEGMENTS];

  if (argc != 6)
    {
      printf("Syntax: gmm-bic no_of_clusters no_of_vals input_learn_file no_of_vals input_test_file\n");
  ...
  m = atoi(argv[4]);     
  printf("Number of input test data values: %d.\n",m);

  test_vector = vector(0,m-1); /* Input test data */
  in_test_vector = vector(0,m-1); /* Input data */
  test_labs = vector(0,m-1);      /* Used later for cluster test labels */

  if ((stream = fopen(argv[5],"r")) == NULL)  {
     fprintf(stderr, "Program %s : cannot open file %s\n",
                       argv[0], argv[5]);
     fprintf(stderr, "Exiting to system.");
     return 1;
  ...
  for (i = 0; i < m; i++)  {
      fscanf(stream, "%f", &in_value);
      test_vector[i] = in_value;
  }

  /*printf("Check on first 10 values of input test data:\n");	
  for (i = 0; i < 10; i++) printf("%5.1f", test_vector[i]);  
  printf("\n");
  ...
  */
  ...
  for (i = 0; i < m; i++) in_test_vector[i] = test_vector[i];
...
/* Determine TEST posteriors.  By Bayes:
     prob(seg-k | ival) = f(ival | seg-k) . prob(seg-k) /
                     sum_r f(ival | seg-r) . prob(seg-r) 
     where LHS is posterior probability of seg-k given the data, ival,
     f(ival | seg-k) is the integrated or marginal likelihood of seg k
  ...
  for (k = 0; k < segment_number; k++) {
      test_cardinality[k] = 0;
      test_sd[k] = sd[k];
      test_segment_probs[k] = segment_probs[k];
  }
  ...
  /*printf("Test Labels are 1, 2, ... no. clusters.\n"); 
  printf("Segment,  Mu,       SD, Mix.Prob, Card.\n");
  for (i=0; i<segment_number; i++) {
      printf("%i, %f, %f, %f,  %d\n",i+1,
      test_mu[i],sd[i],test_segment_probs[i],test_cardinality[i]);
  }*/

   printf("Test Labels follow, n values.\n");
   for (i=0; i<m; i++) printf("%.0f ",test_labs[i]);
   printf("\n");


  free_vector(in_vector, 0, n-1); 
  free_vector(in_test_vector, 0, m-1); 
  free_vector(labs, 0, n-1); 
  free_vector(test_labs, 0, m-1); 
  printf("Finished.\n");
